...
describe('resolveRegionalLanguageProfile', () => {
  ...
  it('does not allow an unrelated device language to override selected language', () => expect(resolveRegionalLanguageProfile({ selectedLanguage: 'pt', deviceLocale: 'en-US', country: 'AO' }).variant).toBe('pt-AO'));
});
describe('RegionalExpressionEngine', () => {
  const engine = new RegionalExpressionEngine([{ expression: 'bué', language: 'pt', variants: ['pt-AO'], regions: ['Angola'], meaning: 'muito', standardEquivalent: 'muito', register: 'slang' }]); const profile = resolveRegionalLanguageProfile({ selectedLanguage: 'pt', country: 'AO' });
