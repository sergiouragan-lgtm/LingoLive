...
export class GeoAwareTutorPipeline {
  ...
  async correct(input: TutorPipelineInput): Promise<CorrectionCandidate[]> {
    ...
    const corrections = await this.correctionAdapter.correct({ systemInstructions: [...(input.geoSystemInstructions ?? []), context.instruction], learnerText: input.learnerText });
    return protectRegionalExpressions({ text: input.learnerText, corrections, profile: context.profile, engine: this.engine, telemetry: this.telemetry });
  }
}

export interface AIEngineOrchestratorLike { correctLearnerText(input: TutorPipelineInput): Promise<CorrectionCandidate[]>; }

export function attachGeoLinguisticCorrectionPipeline(orchestrator: AIEngineOrchestratorLike, options?: { telemetry?: GeoTelemetrySink; engine?: RegionalExpressionEngine }): AIEngineOrchestratorLike {
  const originalCorrection = orchestrator.correctLearnerText.bind(orchestrator);
  const engine = options?.engine ?? new RegionalExpressionEngine(PORTUGUESE_REGIONAL_EXPRESSIONS);
  return {
    ...orchestrator,
    correctLearnerText: async (input) => {
      const context = buildTutorGeoContext(input);
      const corrections = await originalCorrection({ ...input, geoSystemInstructions: [...(input.geoSystemInstructions ?? []), context.instruction] });
      return protectRegionalExpressions({ text: input.learnerText, corrections, profile: context.profile, engine, telemetry: options?.telemetry });
    },
  };
}
