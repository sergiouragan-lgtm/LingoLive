...
export class RegionalExpressionEngine {
  ...
  classifyForCorrection(match: ExpressionMatch, profile: RegionalLanguageProfile) { const expected = match.variants.map(normalizeLocale).includes(normalizeLocale(profile.variant)); return { isRegionalism: true, shouldMarkAsError: false, isExpectedVariant: expected, explanation: match.meaning, standardEquivalent: match.standardEquivalent, register: match.register, regions: match.regions }; }
}
