...
export class AIEngineOrchestrator {
  ...
  async processInteraction(input: LearningInteractionInput): Promise<LearningInteractionResult> {
    ...
    const response = cleanText(await this.executeTutor({ message, userId, level, languageNative, languageTarget, lessonContext: appendGeoInstruction(task, geoContext.instruction), geoSystemInstructions: geoContext.instruction, geoLanguageVariant: geoContext.profile.variant, geoLanguageConfidence: geoContext.profile.confidence, age: student.age, learningGoal: student.learningGoal, ...rawContext }), 8_000);
    ...
    const progress = { interactions, lastActivityAt: timestamp }; const memory = { lastTask: task, lastLearnerMessage: message, lastTutorResponse: response, recentTopics, level, interactions, updatedAt: timestamp };
