...
function nextCefrLevel(current: CefrLevel, averageScore: number, interactions: number): CefrLevel {
  if (averageScore < 85 || interactions < 10) return current;
  return CEFR_LEVELS[Math.min(CEFR_LEVELS.indexOf(current) + 1, CEFR_LEVELS.length - 1)];
}
...
export class AIEngineOrchestrator {
  ...
  async processInteraction(input: LearningInteractionInput): Promise<LearningInteractionResult> {
    ...
    const submittedWords = message.split(/\s+/).filter(Boolean);
    const wordsSubmitted = submittedWords.length;
    ...
    const lexicalDiversity = wordsSubmitted ? Math.round((uniqueWords / wordsSubmitted) * 100) : 0;
    const sentenceComplete = /[.!?]$/.test(message) || wordsSubmitted >= 4;
    const score = Math.min(100, 35 + Math.min(wordsSubmitted, 20) * 2 + Math.round(lexicalDiversity * 0.2) + (sentenceComplete ? 5 : 0));
    ...
    const previousAverage = Number(previousMemory?.averageScore || 0);
    const averageScore = Math.round(((previousAverage * (interactions - 1)) + score) / interactions);
    const nextLevel = nextCefrLevel(level, averageScore, interactions);
    ...
    const memory = {
      ...
      recentTopics, level: nextLevel, interactions, xp, averageScore, updatedAt: timestamp,
    };
    ...
    return {
      response,
      evaluation: { score, completed, wordsSubmitted, lexicalDiversity, sentenceComplete, nextLevel },
      progress,
