...
export interface StudentDashboardExperienceProps {
  ...
  adaptiveMaterials?: Array<{ id: string; title: string; status: string; content?: { summary?: string; sections?: Array<{ heading: string; explanation: string }>; exercises?: Array<{ id: string; prompt: string }> }; gaps?: Array<{ label: string }> }>;
  onGenerateAdaptiveMaterial?: () => void;
  onCompleteAdaptiveMaterial?: (materialId: string, answers: Array<{ exerciseId: string; answer: string }>) => Promise<{ score: number }>;
}
...
export function StudentDashboardExperience(props: StudentDashboardExperienceProps) {
  const { studentName, studentPhotoUrl, selectedLanguage, selectedProficiency, streakData, savedWords, achievements, userProfile, dataState, onRetry, onStartPractice, onNavigate, adaptiveMaterials = [], onGenerateAdaptiveMaterial, onCompleteAdaptiveMaterial } = props;
  ...
  const [adaptiveCompletionError, setAdaptiveCompletionError] = useState<string | null>(null);
  const [isCompletingAdaptiveMaterial, setIsCompletingAdaptiveMaterial] = useState(false);
      ...
      <main id="student-dashboard-content" tabIndex={-1} className="mx-auto max-w-7xl space-y-5 px-4 py-5 sm:px-6 lg:px-8 lg:py-6">
        ...
        {stateContent ?? <>
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            <Panel title="Recomendações adaptativas" icon={<Sparkles className="size-5 text-brand-primary" />}><div data-dashboard-section="adaptive-recommendations" className="space-y-3">{adaptiveMaterials.filter(item => ["ready", "in_progress", "completed"].includes(item.status)).slice(0, 1).map(item => <article key={item.id} className="rounded-ui-md border border-violet-200 bg-violet-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-violet-700">Fascículo personalizado</p><h3 className="mt-1 font-heading text-base font-bold text-ui-text">{item.title}</h3>{item.content?.summary && <p className="mt-2 text-sm text-ui-text-muted">{item.content.summary}</p>}<p className="mt-2 text-xs text-violet-700">{item.gaps?.map(gap => gap.label).join(" · ")}</p><details className="mt-3"><summary className="cursor-pointer text-sm font-bold text-brand-primary">Abrir material</summary><div className="mt-3 space-y-3 border-t border-violet-200 pt-3">{item.content?.sections?.map(section => <section key={section.heading}><h4 className="text-sm font-bold text-ui-text">{section.heading}</h4><p className="mt-1 text-sm text-ui-text-muted">{section.explanation}</p></section>)}{!!item.content?.exercises?.length && <form onSubmit={async event => { event.preventDefault(); if (!onCompleteAdaptiveMaterial) return; setAdaptiveCompletionError(null); setIsCompletingAdaptiveMaterial(true); try { const score = await onCompleteAdaptiveMaterial(item.id, item.content!.exercises!.map(exercise => ({ exerciseId: exercise.id, answer: adaptiveAnswers[`${item.id}:${exercise.id}`] || "" }))); setAdaptiveScore(score.score); } catch { setAdaptiveCompletionError("Não foi possível concluir o fascículo. Tente novamente."); } finally { setIsCompletingAdaptiveMaterial(false); } }}><h4 className="text-sm font-bold text-ui-text">Exercícios</h4><div className="mt-2 space-y-3">{item.content.exercises.map(exercise => <label key={exercise.id} className="block text-sm text-ui-text-muted">{exercise.prompt}<input required value={adaptiveAnswers[`${item.id}:${exercise.id}`] || ""} onChange={event => setAdaptiveAnswers(current => ({ ...current, [`${item.id}:${exercise.id}`]: event.target.value }))} className="mt-1 min-h-10 w-full rounded-ui-md border border-ui-border bg-white px-3 text-ui-text" /></label>)}</div>{onCompleteAdaptiveMaterial && <button type="submit" disabled={isCompletingAdaptiveMaterial} className="mt-3 rounded-ui-md bg-brand-primary px-4 py-2 text-sm font-bold text-white disabled:opacity-60">{isCompletingAdaptiveMaterial ? "A corrigir…" : "Concluir fascículo"}</button>}{adaptiveScore !== null && <p role="status" className="mt-2 text-sm font-bold text-emerald-700">Resultado: {adaptiveScore}%</p>}{adaptiveCompletionError && <p role="alert" className="mt-2 text-sm font-bold text-red-700">{adaptiveCompletionError}</p>}</form>}</div></details></article>)}{model.recommendations.length ? <div className="space-y-3">{model.recommendations.map((item) => <button key={item.id} onClick={() => navigate("adaptive-learning")} className="flex w-full items-center gap-3 rounded-ui-md border border-ui-border p-3 text-left hover:bg-violet-50"><span className="grid size-9 place-items-center rounded-full bg-violet-100 text-violet-700"><BookOpen className="size-5" /></span><span className="flex-1 text-sm font-semibold capitalize">{item.label}</span><ChevronRight className="size-4" /></button>)}</div> : !adaptiveMaterials.length && <CompactEmpty>Pratique para receber recomendações personalizadas.{onGenerateAdaptiveMaterial && <button onClick={onGenerateAdaptiveMaterial} className="mt-3 block w-full font-bold text-brand-primary">Gerar fascículo a partir dos meus gaps</button>}</CompactEmpty>}</div></Panel>
