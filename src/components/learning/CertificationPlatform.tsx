...
export const CertificationPlatform: React.FC = () => {
  ...
  useEffect(() => {
    if (userId) {
      trackEvent('certification_platform_viewed', {
        platformName: 'Certificação LingoLIVE',
        platformType: 'certification_system',
        userRole: role
      });
    }
  ...
  const handleTabChange = (tab: Tab) => {
    setActiveTab(tab);
    if (userId) {
      trackEvent('certification_tab_selected', {
        tabId: tab,
        tabLabels: {
  ...
  const verify = async (event: React.FormEvent) => {
    ...
    try {
      setVerificationResult(await api<Certificate>(`/api/certification/verify/${encodeURIComponent(verificationCode.trim())}`));
      setAuditEvents(await api<AuditEvent[]>("/api/certification/audit"));
      if (userId) {
        trackEvent('certification_verified', {
          verificationCode: verificationCode.trim(),
          certificatePlatformType: 'certification_system'
        });
      }
    }
    catch (err: any) {
      setVerificationError(err.message || "Certificado não encontrado.");
      if (userId) {
        trackEvent('certification_verification_failed', {
          verificationCode: verificationCode.trim(),
  ...
  const revoke = async (certificate: Certificate) => {
    ...
    try {
      await api(`/api/certification/certificates/${encodeURIComponent(certificate.id)}/revoke`, { method: "POST" });
      addToast("Certificado revogado e evento de auditoria persistido.", "info");
      if (userId) {
        trackEvent('certificate_revoked', {
      ...
      addToast(err.message || "Não foi possível revogar o certificado.", "error");
      if (userId) {
        trackEvent('certificate_revocation_failed', {
          certificateId: certificate.id,
          errorMessage: err.message,
          certificatePlatformType: 'certification_system'
        });
