...
const Surface: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = "" }) => (
  <div className={`memory-surface rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 ${className}`}>{children}</div>
);
...
export const LearningMemoryPanel: React.FC<{ userId?: string }> = ({ userId }) => {
  ...
  return (
    <section id="learning-memory-panel" data-memory-theme={isDark ? "indigo-atmospheric" : "editorial-light"} className="learning-memory-theme col-span-1 overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 shadow-sm md:col-span-2 dark:border-slate-800 dark:bg-[#07101f]">
      <header className="memory-chrome flex items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-4 sm:px-6 dark:border-slate-800 dark:bg-[#081323]">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setColorScheme(isDark ? "light" : "dark")} aria-label={isDark ? "Ativar tema Editorial Claro" : "Ativar tema Índigo Atmosférico"} title={isDark ? "Editorial Claro" : "Índigo Atmosférico"} className="memory-theme-toggle grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:text-violet-700 focus-visible:ring-2 focus-visible:ring-violet-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">{isDark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button>
      ...
      <nav aria-label="Navegação da memória" className="memory-chrome grid grid-cols-4 border-t border-slate-200 bg-white px-2 dark:border-slate-800 dark:bg-[#081323]">
...
const Overview: React.FC<{ memory: LearningMemory; saving: boolean; onToggle: () => void; onScreen: (screen: MemoryScreen) => void }> = ({ memory, saving, onToggle, onScreen }) => (
  <div className="space-y-5">
    <div className="memory-hero relative overflow-hidden rounded-3xl bg-[#0b1324] px-5 py-7 text-white sm:px-7 sm:py-9">
...
const Control: React.FC<ControlProps> = ({ memory, setMemory, goals, setGoals, saving, onToggle, onSave, confirmDelete, setConfirmDelete, onDelete }) => <div className="space-y-5"><div className="py-4 text-center"><div className="mx-auto grid h-20 w-20 place-items-center rounded-full border border-emerald-200 bg-emerald-50 text-emerald-600 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-400"><ShieldCheck className="h-9 w-9" /></div><h4 className="mx-auto mt-5 max-w-lg text-2xl font-black tracking-tight text-slate-950 dark:text-white">Os seus dados de aprendizagem pertencem-lhe.</h4><p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Controle como a sua memória é usada para personalizar a sua experiência.</p></div>
  ...
  <button type="button" onClick={onSave} disabled={saving} className="memory-primary-action flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 px-5 py-3.5 text-sm font-black text-white hover:bg-violet-700 disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Guardar alterações</button><div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-xs font-semibold text-emerald-900 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300"><ShieldCheck className="h-5 w-5 shrink-0" /> Desativar preserva os dados e impede o uso.</div>
