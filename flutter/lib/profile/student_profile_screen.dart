...
class _StudentProfileScreenState extends State<StudentProfileScreen> {
  ...
  Future<void> _load() async {
    try {
      final snapshot = await FirebaseServices.firestore.collection('users').doc(widget.user.uid).get();
      final data = snapshot.data();
      _displayName.text = data?['displayName'] as String? ?? widget.user.displayName ?? '';
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await FirebaseServices.firestore.collection('users').doc(widget.user.uid).update({
        'displayName': name,
      ...
      if (mounted) setState(() => _error = 'Não foi possível guardar o perfil.');
    } finally {
          ...
          child: ListView(
            ...
            children: [
              TextFormField(
                ...
                validator: (value) => (value?.trim().length ?? 0) < 2 ? 'Introduza o seu nome.' : null,
              ),
              ...
              TextFormField(
                ...
                textInputAction: TextInputAction.next,
                decoration: const InputDecoration(labelText: 'Código do país', hintText: 'AO', border: OutlineInputBorder()),
                validator: (value) {
                  final code = value?.trim() ?? '';
                  return code.isNotEmpty && code.length != 2 ? 'Use um código ISO de duas letras.' : null;
                },
              ),
              const SizedBox(height: 14),
              TextFormField(
                ...
                textInputAction: TextInputAction.next,
                decoration: const InputDecoration(labelText: 'Idioma nativo', border: OutlineInputBorder()),
              ),
              const SizedBox(height: 14),
              TextFormField(
                controller: _learningLanguage,
                decoration: const InputDecoration(labelText: 'Idioma de aprendizagem', border: OutlineInputBorder()),
              ),
              if (_error != null) Padding(padding: const EdgeInsets.only(top: 12), child: Text(_error!, style: TextStyle(color: Theme.of(context).colorScheme.error))),
              const SizedBox(height: 20),
              FilledButton(onPressed: _saving ? null : _save, child: Text(_saving ? 'A guardar…' : 'Guardar perfil')),
            ],
          ),
        ),
      ),
