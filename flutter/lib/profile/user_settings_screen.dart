...
class NotificationSettingsScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => _Settings(
          user: user,
...
class _SettingsState extends State<_Settings> {
  ...
  Future<void> _load() async {
    try {
      ...
      for (final key in widget.labels.keys) {
        _values[key] = data[key] as bool? ?? false;
      }
    } finally {
  ...
  Future<void> _save() async {
    ...
    try {
      await _ref.set({..._values, 'updatedAt': FieldValue.serverTimestamp()},
          SetOptions(merge: true));
      if (widget.documentId == 'notifications') {
        if (_values.values.any((value) => value)) {
          await NotificationService.instance.activate(widget.user);
        } else {
          await NotificationService.instance.deactivate(widget.user);
        }
      }
      if (mounted)
    ...
    return Scaffold(
        ...
        body: SafeArea(
          ...
          FilledButton(
              ...
              child: Text(_saving ? 'A guardar…' : 'Guardar alterações')),
        ])));
  }
}
