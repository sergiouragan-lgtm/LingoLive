...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Future<void> _checkout(String planId) async {
    ...
    try {
      ...
      if (!await launchUrl(uri, mode: LaunchMode.externalApplication)) throw StateError('Não foi possível abrir o pagamento.');
    } catch (error) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) return const Center(child: Text('Não foi possível carregar a subscrição.'));
          final data = snapshot.data?.data();
          final active = data != null && ['active', 'trialing'].contains(data['status']);
          return ListView(padding: const EdgeInsets.all(20), children: [
            Icon(active ? Icons.verified_user_outlined : Icons.credit_card_off_outlined, size: 64, color: Theme.of(context).colorScheme.primary),
            const SizedBox(height: 16),
            Text(active ? 'Subscrição ativa' : 'Sem plano ativo', textAlign: TextAlign.center, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800)),
            if (active) ...[
              const SizedBox(height: 20),
              Card(child: Column(children: [
                ListTile(title: const Text('Plano atual'), trailing: Text(data['planId']?.toString() ?? '—')),
                ListTile(title: const Text('Próxima cobrança'), trailing: Text(data['currentPeriodEnd']?.toString() ?? '—')),
                ListTile(title: const Text('Cancelamento agendado'), trailing: Text(data['cancelAtPeriodEnd'] == true ? 'Sim' : 'Não')),
              ])),
              const SizedBox(height: 12),
              const Text('A alteração e o cancelamento requerem o portal seguro de faturação.'),
            ] else ...[
